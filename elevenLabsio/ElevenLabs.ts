...
import { axios, type AxiosResponse } from "../deps.ts";
import {
  ElevenLabsRequest,
  ElevenLabsRequestModel,
  ElevenLabsRequestPublic,
  ElevenLabsRequestPublicBody,
...
export class ElevenLabs extends CommonTTS<ElevenLabsRequest> {
  ...
  public async getToken(): Promise<string> {
    ...
    if (!ELEVENLABS_IO_TOKEN) {
      throw Error(
        `Missing ELEVENLABS_IO_TOKEN environement variable, or token in ~/.tts/elevenlab/key.json file, log on https://beta.elevenlabs.io/ and go to Profile Settings to get one`,
      );
    }
  ...
  public async getTts(request: ElevenLabsRequest): Promise<string> {
    ...
    try {
      ...
      >(API_URL, reqData.body, { responseType: "arraybuffer", headers });
  ...
  async getModel(): Promise<ElevenLabsRequestModel[]> {
    ...
    try {
      ...
      const resp = await axios.get<
        never,
        AxiosResponse<ElevenLabsRequestModel[]>
      >(API_URL, { responseType: "json", headers });
  ...
  async getVoices(): Promise<ElevenLabsRequestVoice[]> {
    ...
    try {
      ...
      const resp = await axios.get<
        never,
        AxiosResponse<{ voices: ElevenLabsRequestVoice[] }>
      >(API_URL, { responseType: "json", headers });
