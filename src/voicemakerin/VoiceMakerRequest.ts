...
export function getVoicemakerEffect(
  voiceMakerName: string,
): VoiceMakerEffect[] {
  ...
  if (
    [
      ...
      "ai3-Jenny",
      "ai3-cmn-CN-Xiaoxiao",
    ].includes(voiceMakerName)
  ) {
    result.push("conversational");
  }
  if (
    ["ai1-Amy", "ai3-Jony", "ai3-Aria", "ai3-Jenny", "ai1-es-US-Lupe"].includes(
      voiceMakerName,
    )
  ) {
    result.push("news");
  ...
  if (
    ["ai3-Aria", "ai3-Jenny", "ai3-Jenny", "ai3-Jenny", "ai3-cmn-CN-Yunyang"]
      .includes(voiceMakerName)
  ) {
...
export class VoiceMakerRequest extends CommonTTSRequestAdv {
  ...
  constructor(text: string, voice?: string) {
    super(text, {
      ...
      volume: { min: -20, max: 20 },
      supportedFormat: ["mp3", "wav"],
    });
    if (voice) {
      this.setVoice(voice);
    }
  ...
  public summery(): string {
    return [this.effect, this.voice, this.lang, this.specSummery(), this.text]
      .join(",");
  }
